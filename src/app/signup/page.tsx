...
export default function SignupPage() {
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (resError) {
        setError(
          resError.message || "Failed to create account. Please try again.",
        );
      } else {
      ...
      <div className="w-full max-w-md bg-zinc-950/80 border border-zinc-900 rounded-3xl p-8 backdrop-blur-md shadow-2xl relative">
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="text-zinc-500 text-sm mt-1">
            Get started with Showcase free tier
          </p>
        </div>
        ...
        <form onSubmit={handleSignUp} className="space-y-4">
          ...
          <Button
            type="submit"
            className="w-full bg-white hover:bg-zinc-200 text-black py-6 rounded-xl font-medium mt-2 flex items-center justify-center gap-2 border-0 shadow-lg shadow-white/5"
            disabled={loading || googleLoading}
          >
            {loading ? (
        ...
        <Button
          ...
          variant="outline"
          className="w-full border-zinc-800 text-zinc-300 hover:text-white hover:bg-zinc-900 rounded-xl py-6 flex items-center justify-center gap-3"
          disabled={loading || googleLoading}
        >
          {googleLoading ? (
            <Loader2 className="w-5 h-5 animate-spin" />
